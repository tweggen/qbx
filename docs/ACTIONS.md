...
| `assert-file-identical` | SAssertFileIdenticalAction | testkit/src/sassertfileidenticalaction.cpp | `a`, `b` (the two files; an ABSOLUTE path is used as given — that is the point, a golden produced by another process has to be nameable — a path with a separator is relative to the PROCESS working directory, a bare name resolves in the test output directory where renders land), `startFrame` = "0", `frameCount` = "-1". Default (`frameCount` = -1, `startFrame` = 0) compares the WHOLE files byte for byte, headers included — the `cmp` the determinism gate used to run by hand. Given a range, both files are parsed as RIFF/WAVE, their formats must agree, and only those frames of the `data` chunk are compared |
