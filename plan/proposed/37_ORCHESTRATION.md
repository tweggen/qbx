...
- **Gate (ACs):**
  ...
  - **Not gated (PR body):** stereo (sink mono until 36-B5; channel 0 only) —
    **stereo gated since 2026-08-17 by `instrument_stereo_render.qxa`**, which
    walks DirectGen / GenFold / the refused narrow-generator row / MonoSpread per
    channel out of rendered files (WideGen stays engine-level only: no in-repo
    instrument has three main outputs); real third-party instruments (demo only);
    render-vs-playback identity; arp → instrument in-app (P9).
...
### P8 — Live MIDI input + recording  *(EXECUTED as proposal 21 L2 + L4, 2026-08-17/18 — see `21_ORCHESTRATION.md`)*
...
| P3b instrument slot + event feed | ☑ | 2026-08-16 | `feat/36-p3b-instrument-slot` | AC1–AC7b, AC9, AC10 green. **ctest -j4: 157/157 run passed, 160 registered** (153 + 7 new `instrument_*` cases), 3 `au_*` disabled; layering + logging clean; goldens untouched and green. Generator rows `DirectGen`/`MonoSpread`/`GenFold`/`WideGen`; the head insert keeps its input and the PROCESSOR sums it (`x + 0.0f == x` gated by a 576044-byte compare); reset + chase + pre-roll K per D4; `forgetContinuity()` exposed for P3c. Two decisions not in the brief: the MIDI(0..127) → ABI([0,1]) conversion lives in the processor (`twNormalizeForAbi`, plugins inv. 39), and `insert-plugin` gained an `isInstrument` attribute (written only when true). **AC5's mid-render bypass toggle is gated in `plugins_test::testGeneratorSlot`, NOT in a qxa** — a render always starts at the range start and every non-contiguous page is a reposition, so a render-based assertion would pass on the buggy code. Stereo, real third-party instruments and render-vs-playback identity NOT gated — **stereo gated since 2026-08-17 by `instrument_stereo_render.qxa`** (see STATE.md 2026-08-17). See STATE.md 2026-08-16 |
| P3c render barrier + determinism | ☑ | 2026-08-16 | `feat/36-p3c-render-barrier` | AC1–AC4 + AC2c green. **ctest -j4: 160/160 run passed, 163 registered** (160 + 2 new `instrument_*` cases + the cross-process CMake driver), 3 `au_*` disabled; layering + logging clean; goldens untouched. `SApplication::beginRun(pos)` = `forgetContinuity()` THEN `invalidateRenderPathRange(pos, INT64_MAX)` per instrument track, from FOUR main-thread call sites (startRender before the session thread; `SMainWindow::startPlaying`, `setPlaybackRunning` and startRecording's monitoring playback, each immediately before `startOutput()`). The walk is `sinstruments::collectInstrumentTracks()` — a walk, not a registry. AC2(b) band 0.388957 ±10 %, render and capture identical to every digit; forced K=4096 reads 0.215076 (1.81× off). Barrier cost: **2 re-rendered pages per play start**, ~20 ms. **Recorded honestly: the AC1 case also passes with the barrier removed** — P3b's double-render debt makes every instrument page position-pure, so nothing can leak yet; these are regression gates on a guarantee. Seek-during-playback and loop-wrap splices NOT gated. See STATE.md 2026-08-16 |
...
| P5 automation model + engine | ☑ | 2026-08-16 | `feat/36-p5-automation` | AC1–AC8 green. **ctest -j4: 162/162 run passed, 0 failed, 165 registered** (160 + 5 new `automation_*` cases), 3 `au_*` disabled, in 218 s. An earlier run hit the PRE-EXISTING `clip_properties_actions` teardown SEGFAULT named in CLAUDE.md (5/5 clean in isolation; green in the final run). AC8 sweep **50/50 at each of SMARAGD_REVAL_WORKERS {1,4,8,16}, 200/200**. Layering + logging clean; goldens untouched; `action_roundtrip_test` 130 actions. New fixture `tests/test_autosaw.wav` (480 Hz, period exactly 100 frames) + its generator. Three decisions not in the brief: `self:Volume` interpolates linearly IN dB (the only reading of "dB-linear in fader space" a module that may not include an app header can implement, and the only one under which Trim is a dB SUM); point times are whole FRAMES, not `Fraction`, because `twAutomationCurve` breakpoints are int64; the verbs live in `main/objects/track`, not `main/actions`, because only the `param:` owner needs a concrete type. One regression found BY the gate and fixed: access specifiers inside `strack.h`'s `public slots:` block demoted `trackEventClipChanged` and silently broke every event-clip edit. Mode UI / Touch-Latch-Write recording (P6), pan (36-B5), stereo and placement-scope lanes (32) NOT gated — **stereo gated since 2026-08-17 by `automation_stereo.qxa`** (a `self:Volume` ramp and a `param:` step on both channels; see STATE.md 2026-08-17). See STATE.md 2026-08-16 |
| P6 automation UI | ☑ | 2026-08-16 | `feat/36-p6-automation-ui` | AC1-AC4 green. **ctest -j4: 168/168 run passed, 0 failed, 171 registered** (139 -> 142 qxa files + the xproc driver, plus 28 unit tests), 3 au_* disabled, in 158 s; action_roundtrip_test 132 actions; layering + logging clean; goldens byte-identical BY CONSTRUCTION (UI only). New `main/timeline/src/sautomationlane.{h,cpp}` (875 lines) holds the whole feature, including the definitions of the five `SStdMixerView` members that are automation code - **`sstdmixerview.cpp` 4458 -> 4494, +36 against a budget of 100** (AC4). `STrackRow::subKind {None,Take,Automation}`; ONE `pruneUiState()` walk for all four per-track UI-state sets (there was none before). The head "A" button governs EVERY lane the track owns, in one undo macro; the mode is `Amode=` in `describeHead()`, appended after `name=` so every P4 `contains=` still matches. `SAutomationRecorder` lives in `main/shell` because app/timeline and app/pluginui both feed it and cannot see each other. **One PRE-EXISTING P5 bug found by the gate and fixed in its own commit**: `SAutomationLane::setPoints()` used a non-stable sort plus `std::unique`, so `add-automation-point` on an occupied frame silently DROPPED the new value instead of replacing it. Goldens byte-identical BY CONSTRUCTION (UI only). NOT gated: plugin-gesture punch-in (the ABI surfaces it only into `twEventOut` at freeze time, with no consumer and no native editor), Delete over a marquee (a QAction shortcut, not synthesisable), Latch/Write passes, the read-value display, pixel exactness. See STATE.md 2026-08-16 |
...
| P8 live MIDI input (outline) | ☑ | 2026-08-18 | proposal 21 L2 (P8a) + L4 (P8b) | Executed AS PART OF PROPOSAL 21: L2 = live instruments (`twLiveEventSource`, MIDI-in fan-out rings, thru, keyboard port), L4 = MIDI recording (`SMidiRecorder`, `add-midi-take`, `place-midi-recording`, modes, loop takes, input quantise). Metronome landed in 21 L5. Gates and numbers in `21_ORCHESTRATION.md` §6. |
