...
- **Status 2026-08-18: COMPLETE** — measurement, runner and the Options
  wizard. The dialog proposes and never applies: a successful measurement is
  shown first, "Yes" only fills the spin box, and the value is not written
  until the dialog is applied — two deliberate steps between a measurement and
  a timing constant that shifts every take recorded afterwards.** And one
  finding worth carrying into any similar work: **a synthetic gate could not
  find the defect that mattered.** The refusal criterion was peak-to-noise
  alone, which Gaussian noise passes; a real unconnected input produced a
  confident 345 ms from a 0.45 %-of-full-scale spike over a very quiet floor,
  because a spike satisfies any purely relative test. The fix is an ABSOLUTE
  threshold against what was emitted, and the regression test now asserts both
  halves — that the relative test alone IS fooled, and that the absolute floor
  refuses.
- **Positive case measured 2026-08-18** (US-16x08, OUT 1 patched to IN 1):
  round trip **1084 frames = 22.58 ms** against a driver-claimed 1039, i.e. the
  driver is honest to within **45 frames** and the residual a user would be
  offered is **+0.94 ms**. It also exposed the next thing: that pass sat only
  **1.5x above the refusal floor** (peak 0.0076 for a 0.5 probe, a 36 dB loss),
  so a little less input gain would have refused a perfectly good cable with
  "check the cable" — the wrong advice. The result now carries `headroom` and a
  `LoopbackLevel` (None/TooWeak/**Weak**/Good/Hot) with advice written for a
  human, because a pass with no margin and a comfortable pass are both `found`
  and only one deserves trust.
...
| L6a loopback latency calibration | ☑ | 2026-08-18 | | measurement, runner and Options wizard; MEASURED on hardware (1084 vs a claimed 1039, residual +0.94 ms). The DIALOG has no headless gate — no verb builds the Audio page off screen |
