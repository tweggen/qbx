...
| E4 | (L1b) D3's Closure master mode is REFUSED, not run: the plan builder can express it, but the RT still adds the frozen root page whenever the frozen lane is PLAYING and nothing reads `twLivePlan::masterLinear`. Unreachable today — the master is a unity `twMixer` + identity `twRewire` by construction. | Whoever adds a master insert chain lands on the log line; the fix is a `twSpeaker` flag that pulls-and-discards the root while a Closure plan is live. |
| E5 | (L1b) The disarm releases processor ownership BEFORE the re-wire (else the first re-summed page freezes as silence and the epoch gate flips the RT onto it), so during the hand-back tail the pump and the freeze path both render the departing processors. | Bounded to the tail (~256 ms), mutex-serialised; a stateful insert can carry a small artefact across it. Recorded in `main/shell/CONTRACT.md`. |
