...
| `devices_input_test` | **every block of a paced `FileAudioInput` run is DELIVERED within 2 ms of its due time** (96 blocks, 21.333 ms period) | 0.11–0.15 ms on an idle box; **1.3–5.2 ms at load average ~9, failing 6 runs in 8**. Measured 2026-09-16 with MAIN's own unmodified binary, so a failure here is the box, not the branch — confirm before reading it as a regression. Already `RUN_SERIAL`, which did not save it: the load was OUTSIDE the ctest run |
...
| **Every meter draws a dB TICK SCALE itself** (QBX-101) | `SLevelMeter` is the one widget behind every mount, so the scale needed no mount edits. Ticks use `dbToPx()`, the bars' own mapping; minor ticks are gray notches HALF of each lane wide (a stereo head's lanes are 3 px, and a full-width notch would hide both readings); the 0 dB tick is white, 2 px thick, full width; the step thins from 3 to 24 dB to keep ticks 4 px apart. Gate: `levelmeter_ticks_test`, a PIXEL test (`timeline/CONTRACT.md` inv. 11b). NOT gated: how the scale LOOKS against the zone colours |
...
| Value domains are the TARGET's own: `self:Volume` in **dB**, `self:Muted` 0/1, `param:<id>` in the plugin's **host-facing** domain (normalized for VST3), `cut:Gain` a **LINEAR** factor | dB for the fader because that is its unit and because Trim's "static value × curve" is then a dB SUM; linear for a clip envelope because a fade-out has to reach EXACTLY zero. `param:` matches `set-plugin-param` by construction. `Rate`/`Stretch` are not automatable (they change duration). **`self:Pan` LANDED IN PROPOSAL 49 M3 (QBX-111)** and its domain is the CONTROL's own [-1, 1] (49 D4) — not a gain, so Trim SUMS in the pan domain the way the fader's Trim sums in dB, and the curve is interpolated in the pan VALUE with the law applied to the RESULT (49 D3). This row read "`self:Pan` is still absent" for a year. |
...
moves both channels while keeping the skew's ratio of 4. **PAN IS NO LONGER ON
THIS LIST** — proposal 49 built it (M1 clip pan, M2 track pan, M3 the `self:Pan`
lane), and its own gates are `qxa.automation_pan_ramp`,
`qxa.automation_pan_trim_read` and `qxa.automation_pan_stereo`. NOT gated:
...
changed. Invariants: `main/timeline/CONTRACT.md` inv. 34-35,
