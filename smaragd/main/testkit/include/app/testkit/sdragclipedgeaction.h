 ...
 * - edge:   "end" (right edge) or "start" (left edge)          (default "end")
 * - toTime: drop position, in frames
 * - half:   "lower" = extend/trim, "upper" = the loop half of the
 *           right-edge band                                     (default "lower")
 *
 * Limits: modifier gestures (Ctrl time-stretch, Alt slip, Ctrl duplicate) are
 * NOT drivable — the handlers read the live keyboard through
 * QGuiApplication::keyboardModifiers() rather than the event's modifiers.
 * The drop lands on a pixel boundary, so the resulting time is quantised to the
 * view's current zoom; assert on ranges, not exact frame counts.
 */
class SDragClipEdgeAction : public SAction {
public:
    SDragClipEdgeAction() = default;
    SDragClipEdgeAction( int track, int clip, bool grabEnd, offset_t toTime,
                         bool upperHalf );
