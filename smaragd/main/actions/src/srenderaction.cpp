...
SApplyResult SRenderAction::apply(SProject *project)
{
    ...
    // go.
    //
    // So the budget has TWO terms, answering two different questions, and
    // neither subsumes the other:
    //
    //   * how slow is this machine?  The base is overridable. The default is
    //     unchanged, which keeps the app's behaviour and any hand-run case
    //     exactly as before; the qxa suite raises it (see
    //     smaragd/CMakeLists.txt) and relies on CTest's own per-test TIMEOUT as
    //     the real hang guard -- a bound that is about the test, not about one
    //     action inside it.
    ...
    int maxWaitMs = 30000;  // base: the machine-speed term
    {
